...
def fetch_calendarific_holidays(
    ...
    try:
        ...
        for holiday in holidays:
            ...
            elif any(t in holiday_type for t in ['Observance', 'Season', 'Local observance']):
                fun.append(name)
            elif 'Common local holiday' in holiday_type:
                fun.append(name)
...
def fetch_holidays(
    ...
    # 3. Try Checkiday for fun observances
    ...
    # If no fun observances provider available
    if not calendarific_api_key and not checkiday_api_key:
        ...
        # APIs configured but no observances found
        fun_observances = []
